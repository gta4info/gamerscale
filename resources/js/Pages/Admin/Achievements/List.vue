<template>
    <AdminLayout>
        <template #title>Список ачивок</template>

        <v-data-table :items="achievements" :headers="headers" :search="search"
                      no-data-text="Добавленных ачивок не найдено">
            <template v-slot:top>
            ...
            <template v-slot:item.actions="{ item }">
                <inertia-link :href="`/achievement/view/${item.id}`">
                    <v-icon
                        class="me-2"
                        size="small"
                    >
